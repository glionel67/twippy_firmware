...
uint16_t get_adc_imot1(void) {
	return get_adc_imot_value(ADC_IMOT1_CHANNEL);
}

uint16_t get_adc_imot2(void) {
	return get_adc_imot_value(ADC_IMOT2_CHANNEL);
}

void get_adc_imot12(uint16_t* _i1, uint16_t* _i2) {
	(*_i1) = (*_i2) = 65535;
	(*_i1) = get_adc_imot_value(ADC_IMOT1_CHANNEL);
	(*_i2) = get_adc_imot_value(ADC_IMOT2_CHANNEL);
}
...
void get_adc_imot12_ma(uint16_t* _i1, uint16_t* _i2) {
	...
	adc = get_adc_imot_value(ADC_IMOT1_CHANNEL);
	if (adc != 0xFFFF) {
	...
	adc = get_adc_imot_value(ADC_IMOT2_CHANNEL);
	if (adc != 0xFFFF) {
		volt = ((uint32_t)adc*(uint32_t)ADC_IMOT_VREF)/((uint32_t)ADC_IMOT_RESOLUTION);
		ma = (volt*IMOT_RATIO_NUM)/IMOT_RATIO_DEN;
		(*_i2) = (uint16_t)ma;
